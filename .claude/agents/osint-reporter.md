...
- 원문 인용은 원어를 보존한다 (특히 영문 NASA/ESA/Maxar 분석)
...
- **위성 출처 미확인 이벤트는 본문이 아닌 "미검증 의혹" 섹션으로 분리** (CLAUDE.md 도메인 규칙)
- **다중 위성 교차검증된 이벤트는 별도 강조 섹션** 배치
- **한반도 GeoFocus** 섹션을 본문 상단부에 별도 배치
- 전후 비교(before/after) 영상이 있는 이벤트는 본문에 명시한다

## KG 시각화 원칙
- 노드: 엔티티 (클래스별 이모지로 구분)
  - Event: ⚡  Satellite: 🛰  Sensor: 📡  Organization: 🏛  Location: 📍  Phenomenon: 💡  Country: 🌐
- 엣지: 관계 (라벨 포함)
- 새로 발견된 노드/엣지는 굵은 테두리로 강조
- 추론된 관계는 점선으로 표시
- 시각화가 복잡해지면 도메인(Disaster/HumanActivity/Climate/AgriMarine/Defense/Humanitarian)별로 분리하여 여러 다이어그램을 생성
