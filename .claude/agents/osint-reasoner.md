...
- **다중 위성 교차검증**(multi_satellite_confirmation) 추론을 우선 수행한다 — EO 도메인 핵심 신호
- **Sensor capability** 추론(SAR/TIRS/trace_gas/hi-res)을 적극 적용하여 현상-센서 적합성을 검증한다
- **cascading_disaster** 추론으로 태풍→홍수, 지진→산사태 등의 인과 사슬을 표시한다
- **korea_geo_focus** 가산점은 KR/KP 국가 코드 매칭으로 자동 적용한다
