...
1. `ontology/instances.json`에서 최근 7일 내 추가/언급된 주요 엔티티명을 추출
2. 중요도가 높은 엔티티(참조 횟수 3회 이상)를 보조 키워드로 추가
3. 확장 키워드는 기본 키워드와 조합하여 검색 (예: "위성 + 위성명", "이벤트 + 지역명")
4. **EO 도메인 특화 확장**: 시드된 위성·센서 이름(Sentinel-1, Landsat 9, KOMPSAT-3A, MODIS, Sentinel-5P TROPOMI 등)과 도메인 키워드(wildfire, flood 등)를 곱집합으로 결합
